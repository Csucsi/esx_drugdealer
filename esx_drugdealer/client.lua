...
local ped = nil;
local isNear = false;
local resource_name = "rl-drugdealer";
		...
		if isNear then
			...
			if IsControlJustReleased(0, 38) then
				if not isPurchased then 
					TriggerServerEvent('rl-drugdealer:checkjob')
				else
...
function buySpeed()
	TriggerServerEvent('rl-drugdealer:buyspeed')
end

function buyLSD()
	TriggerServerEvent('rl-drugdealer:buylsd')
end

function buyEcstasy()
	TriggerServerEvent('rl-drugdealer:buyecstasy')
end
...
RegisterNetEvent('rl-drugdealer:kindjob')
AddEventHandler('rl-drugdealer:kindjob', function()
	MenuDrugs();
end)
RegisterNetEvent('rl-drugdealer:notkindjob')
AddEventHandler('rl-drugdealer:notkindjob', function()
	ESX.ShowNotification("~r~You are not in right faction!")
end)
RegisterNetEvent('rl-drugdealer:limiter')
AddEventHandler('rl-drugdealer:limiter', function()
	Limiter();
end)